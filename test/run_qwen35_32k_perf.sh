...
# 只采集 inference (test_xllm.py) 期间的 NPU 系统级数据 / Python 客户端执行数据
# 通过 msprof 包裹 python 调用，极大避免采集 load model 和初始化阶段的长空泡。
# 注意如果要动态 profile Server，可加上 --application="python test_xllm.py..." --sys-hardware 等配置 
# 这里根据需求直接在对应的这部分加上 msprof

/usr/local/Ascend/ascend-toolkit/latest/bin/msprof --output="$PROF_DIR" --application="python test_xllm.py \
  --backend xllm \
  --dataset-name random \
